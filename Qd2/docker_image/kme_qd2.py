...
dic_ips = {
    "10.4.32.221": ["10.4.32.207", "app1@qd2node1", "10.4.32.96"],
    "10.4.32.96": ["10.4.32.121", "app2@qd2node2", "10.4.32.221", "10.4.32.144"],
    "10.4.32.144": ["10.4.32.106", "app3@qd2node3", "10.4.32.96", "10.4.32.227"],
    "10.4.32.227": ["10.4.32.124", "app4@qd2node4", "10.4.32.144", "10.4.32.236"],
    "10.4.32.236": ["10.4.32.107", "app5@qd2node5", "10.4.32.227"]
}
        ...
        while True:
            ...
            with conn:
                ...
                while recibiendo:
                    data = conn.recv(1024)
                    espacio = " ".encode()
                    prev_ksid, dst_bytes, size_bytes = data.split(espacio)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            with conn:
                ...
                recibiendo = True
                while recibiendo:
                    end_to_end_encrypted = conn.recv(1024).decode()
                    recibiendo = False
                received_ete.set()
    ...
    def do_GET(self):
        ...
        if src == ip: #Case in which the application made the request to the "first" KME 
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                espacio = " ".encode()
                data = ksid1+espacio+dst_bytes+espacio+size_bytes
                s.sendall(data)
            time.sleep(2)
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.sendall(end_to_end_encrypted.encode())

        elif dst==ip: #Case in which the application made the request to the "last" KME
            #Wait for a ksid
            new_ksid.wait()
            response = qclient.open_connect("app5@qd2node5", "app6@qd2node6", key_chunk_size=size, ttl=100000, ksid=prev_ksid)
...
def forwarding_ete():
    ...
    while True:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            size_bytes = str(size).encode()
            espacio = " ".encode()
            data = ksid2+espacio+dst_bytes+espacio+size_bytes
            s.sendall(data)
        time.sleep(2)
